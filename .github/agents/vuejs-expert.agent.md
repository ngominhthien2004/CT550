...
argument-hint: 'Describe target views/components/stores, expected UX behavior, and API endpoints involved.'
handoffs:
  - label: Implement Backend API
    agent: API Architect
    prompt: 'Implement or update backend API contract required by this frontend change using existing backend conventions.'
    send: false
  - label: Analyze MongoDB Performance
    agent: mongodb-performance-advisor
    prompt: 'Analyze query/index performance for backend paths used by this frontend flow and suggest optimizations.'
    send: false
---
