<template>
  <div v-if="visible" class="modal-overlay" @click.self="$emit('close')" @keydown.enter.prevent="$emit('close')" @keydown.space.prevent="$emit('close')" tabindex="0" role="button">
    <div class="modal-container" role="dialog" aria-modal="true" aria-labelledby="report-modal-title">
      <div class="modal-header">
        <h3 id="report-modal-title">Report Artwork</h3>
        <button type="button" class="btn-close" @click="$emit('close')" aria-label="Close">&times;</button>
      </div>
      <form @submit.prevent="submitReport">
        <div class="modal-body">
          <p class="text-muted mb-3">Why are you reporting "{{ artwork?.title }}"?</p>
          <div class="form-group mb-3">
            ...
            <select v-model="reason" class="form-select" required aria-label="Report reason">
              ...
              <option value="nsfw">Missing age rating / NSFW</option>
...
<script setup>
import { ref } from 'vue'
import { reportArtwork } from '@/services/api'
...
async function submitReport() {
  ...
  success.value = ''
  try {
    await reportArtwork(props.artwork._id, {
      reason: reason.value,
