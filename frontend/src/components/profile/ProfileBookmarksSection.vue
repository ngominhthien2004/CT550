...
const filteredBookmarks = computed(() => props.bookmarks.filter(b => b && b.artwork))
</script>
...
<template>
  <section class="bookmarks-panel" aria-label="Bookmarks section">
    ...
    <div v-else-if="filteredBookmarks.length" class="bookmark-grid">
      <ArtworkCard v-for="bookmark in filteredBookmarks" :key="bookmark._id" :item="bookmark.artwork" />
    </div>
    ...
    <button
      v-if="bookmarks.length >= limit && hasMore"
      type="button"
      class="load-more-btn"
      :disabled="loading"
